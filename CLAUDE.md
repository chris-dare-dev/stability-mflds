...
| `exceptional_existence` | E15-M1 / G18+. `exceptional_refutation(r, c1, surface)` → `ExceptionalRefutation` — PROVEN necessary-condition battery for "an exceptional bundle of this character exists" (the §11-conjecture tool): the `ρ_gen ≥ 2` prioritary route + the rigid-factor obstruction (`prop-mukai` only — `thm-rigidSplit` is del Pezzo-only). Refutation is proven; a pass is honestly inconclusive. CORRECTIONS §21. |
| `block_kronecker` | E15-M3 / G18+. The Conjecture A falsification harness: `classify_generic_filtration(r, c1, ch2, surface)` → `FiltrationPattern` (factor semiexceptionality; flags a length-≥3 filtration with ≥ 2 non-semiexceptional factors — an immediate counterexample) and `block_decomposition(v1, v2, surface)` → `BlockWitness` — the exact ℤ-span search over the §8 collection family (extended `ℓ` range, all twists, the F₀ ruling swap; χ-orthogonality-filtered). `None` is search-bounded, never a counterexample claim. |
| `conjectural_delta` | E15-M4 / G18+. `delta_conjectural(nu, m, surface)` → `DeltaConjectural` — the Conjecture A-gated `δ_m^{μs}` evaluator: `max(dlp_envelope part, thm-deltaKronecker over the twist/swap orbit)`; rigor `PROVEN` only where the certified-sharp DLP part dominates (the `−K` ray), else `CONJECTURAL` with the hypotheses named on the certificate. |
