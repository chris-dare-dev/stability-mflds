...
| `delta_sharp` | E14-M1 / G18. The sharp Bogomolov function `δ_m^{μs}(ν)` (`def-deltass`): `mu_stable_exists(r, nu, Delta, surface)` — a PROVEN decision procedure for slope-stable existence at rational `m` (chamber-sample certificate; honest `None` only on `Δ = 1/2, r ≥ 2`) — and `delta_mu_stable(nu, m, surface, max_rank)` → `DeltaSharp(lower, upper, exact)` certified sandwich; `polarization_index` / `surface_with_index`. The inf need not be attained — see CORRECTIONS §17. |
