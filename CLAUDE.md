...
python examples/demo.py --figures   # also write figures/*.{html,png}
# figure-review gallery (see .claude/launch.json; use `python` on Windows):
...
| `mukai` | K3 Mukai lattice. `MukaiVector(r, l, s)`, `pairing`, `self_pairing`, `moduli_dim`, `is_spherical`, `is_isotropic`, `classify_wall` → `WallClassification` (Bayer–Macrì Thm 5.7). |
...
- **[`docs/FRONTEND_REVAMP_PROMPT.md`](docs/FRONTEND_REVAMP_PROMPT.md)** — the
  spec for the visualization revamp (full public API, figure critique,
  recommended stack: matplotlib + plotly SVG only, never `go.Scattergl`/WebGL,
  and acceptance criteria). Scope is the viz layer only — do not touch the math
